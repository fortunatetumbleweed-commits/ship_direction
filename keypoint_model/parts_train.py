...
def validate(model, xr, degs, names, L0):
    ...
    for b in range(xr.shape[0]):
        ...
        _, open_mask = parts.mr.open_mask_from_crop(img)        # cover visible green, not open water
        heading, _ = parts.part_pose(P, L0, open_mask=open_mask, ship_mask=parts.ship_green(img))
